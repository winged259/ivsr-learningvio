...
class DownscaleFlow(object):
    ...
    def __call__(self, sample): 
        ...
        if self.downscale!=1 and 'flow' in sample.keys():
        ...
        if self.downscale!=1 and 'intrinsic' in sample.keys():
            sample['intrinsic'] = cv2.resize(sample['intrinsic'], 
                (0, 0), fx=self.downscale, fy=self.downscale, interpolation=cv2.INTER_LINEAR)

        # if self.downscale!=1 and sample.has_key('fmask'):
        if self.downscale!=1 and 'fmask' in sample.keys():
...
def dataset_intrinsics(dataset='tartanair'):
    ...
    elif dataset == 'flightmare': ## QuyenPV ##
        focalx, focaly, centerx, centery = 180.0, 320.0, 180.0, 180.0
...
def plot_traj(gtposes, estposes, vis=False, savefigname=None, title=''):
    ...
    plt.subplot(111)
    plt.plot(gtposes[:,0],gtposes[:,1], linestyle='dashed',c='k') #world frame
