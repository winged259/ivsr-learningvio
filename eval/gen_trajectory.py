...
def gen_trajectory_rel_iter(model, dataloader, prop_lstm_states, initial_pose=np.eye(4, 4)):
    ...
    for i, data in enumerate(dataloader):
        ...
        # images = data[1].cuda()
        meta_data, images, imu_data, prev_state, T_imu_cam, gt_poses, gt_rel_poses, intrinsic_layer = data

        lstm_states = lstm_states if prop_lstm_states else None
        # we only care about the results from the VO front ends here
        vis_meas, vis_meas_covar, lstm_states, _, _,  = model.forward(images.cuda(),
                                                                       imu_data.cuda(),
                                                                       lstm_states,
...
def gen_trajectory_abs_iter(model, dataloaders):
    ...
    for i in range(0, max_length):
        ...
        # meta_data, images, imu_data, prev_state, T_imu_cam, gt_poses, gt_rel_poses, intrinsic_layer
        ...
        intrinsic_layer = torch.stack([torch.squeeze(d[7], 0) for d in data_list]).cuda() 
        ...
        vis_meas, vis_meas_covar, est_poses, est_ekf_states, est_ekf_covars = \
            model.forward(images, imu_data, prev_pose, prev_state, prev_covar, T_imu_cam, intrinsic_layer)
